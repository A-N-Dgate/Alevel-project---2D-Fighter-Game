...
def level(screen,player,level):
    ...
    framerate,action_stack,moves,counter_moves,play,defeated,time_left,keydown,level_start_time = level_values()

    if not defeated and time_left:
        defeated,time_left,start_time = main_game(defeated,time_left,screen,framerate,player,enemy,moves,counter_moves,action_stack,keydown,character_group,healthbar_group,number_group,timer_numbers,level_start_time,backgrounds)
    ...
    if not time_left and play:
        while play:
            ...
            play = time_out_update(screen, backgrounds, player, enemy, ticks, character_group, healthbar_group, start_time, end_level_images[2], 0)
            if not play:
                return False
...
def practice_mode(screen,player):
    ...
    moves, action_stack, keydown, framerate, previous_list, exit_game, mouse_x, mouse_y = practicemode_game_setup()
    ...
    while not exit_game:
        ...
        action_stack, keydown, exit_game, mouse_x, mouse_y = setqueue_eventcheck(action_stack, moves, player, keydown, exit_game,exit_button, mouse_x, mouse_y)
        keypressed = check_true(keydown)

        if not player.get_animating():
            mode, action_stack = character_reaction(player, action_stack)

        dummy_reaction(dummy)
        check_attack_dummy(player, dummy)
        stay_on_screen(player,dummy)

        previous_list = practice_update(screen,backgrounds,player,(mode,keypressed,action_stack,previous_list),dummy,ticks,(healthbar_group,button_group,character_group,),(mouse_x,mouse_y))
    pygame.mixer.Channel(2).play(pygame.mixer.Sound(os.path.join("sfx", "select.wav")))


def main_game(defeated,time_left,screen,framerate,player,enemy,moves,counter_moves,action_stack,keydown,character_group,healthbar_group,number_group,timer_numbers,level_start_time,backgrounds):
    while not defeated and time_left:
        framerate.tick(30)
        ticks = pygame.time.get_ticks()

        action_stack, keydown = set_stack(action_stack, moves, player, keydown)
        keypressed = check_true(keydown)

        if not player.get_animating():
            player_mode, action_stack = character_reaction(player,action_stack)
...
def arcade_mode(screen, player, player_data, level_number):
    #fist called with level_number = 1
