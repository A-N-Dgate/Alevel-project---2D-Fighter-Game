...
class save_data():
    ...
    def get_player_id(self):
        SQL = "SELECT * FROM Players WHERE Players.name = '%s'" %(self.get_name())
        ...
        try:
            ...
            for row in self.c.execute(SQL):
                if row[1] != self.get_level_number():
                    ...
                    #print("Name: %s, level: %d" %(self.get_name(),self.get_level_number()))
        except:
            print("name not found, create new data file")
    ...
    def save(self):
        try: #checking if the player is already in the player table
            SQL = "SELECT * FROM Players WHERE Players.name = '%s'" %(self.get_name())
            for row in self.c.execute(SQL):
                name = row[1]
                playerID = row[0]
            ...
            SQL ="INSERT INTO Players(player_id,name) VALUES (NULL, '%s')" %(self.get_name())

            self.c.execute(SQL)
            self.conn.commit()
            print("\nPlayer added to the player table")
