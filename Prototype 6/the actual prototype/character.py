...
class character(my_sprite):
    ...
    def update(self,current_time, rate, x, y, mode, keydown): 
        ...
        if self.frame != self.old_frame: 
            ...
            if mode != "":
                pos = self.movement(mode, x, y, keydown)
                x = pos[0]
                y = pos[1]
...
class Trunks(character):
    def __init__(self,target):
        ...
        self.values = [self.default_y, self.highest_y, self.down_value, self.select_x]
        self.special_moves = {"shining_slash": ["block","chop", "move_right"]}
    ...
    def shining_slash(self):
        ...
        self.animating = True
        self.blast = True
        self.ki_blast = shining_slash(self)
...
class Vegeta(character):
    def __init__(self,target):
        ...
        self.values = [self.default_y, self.highest_y, self.down_value, self.select_x]
        self.special_moves = {"galick_gun": ["block", "kick", "move_left"]}
...
class Cell(character):
    def __init__ (self, target):
        character.__init__(self, "Cell", target)
        #character values
        self.health = 200
        ...
        self.defence = 20
        #getting the spritesheet
        path = os.path.join("spritesheets","characters", "Cell","CellSheet.png")
        self.load(path, 144, 130, 83)
    ...
    def set_box_lists(self,x ,y): 
        ...
        if not self.reversed:
            self.punch_boxes = [None,None, None, pygame.Rect((x + (self.hitbox_width-15)),(y + 62),10,10), None]
            self.kick_boxes = [None,pygame.Rect((x+70),(y+(self.hitbox_height - 45)),10,10),pygame.Rect((x+ 65),(y+(self.hitbox_height - 82)),10,10),None, None]
            self.chop_boxes = [None,None,None,pygame.Rect((x+(self.hitbox_width - 20)),(y+65),10,10),pygame.Rect((x+(self.hitbox_width - 20)),(y+65),10,10),None]
            self.low_punch_boxes = [None,None,pygame.Rect((x+(self.hitbox_width - 20)),(y+50),10,10),pygame.Rect((x+(self.hitbox_width - 18)),(y+50),10,10),None]
            self.low_kick_boxes = [None,None,pygame.Rect((x+(self.hitbox_width-20)),(y+ (self.hitbox_height + 20)),10,10),None,None,None]
            self.uppercut_boxes = [None,None,pygame.Rect((x+ (self.hitbox_width -20)),(y+70),10,10),pygame.Rect((x+(self.hitbox_width - 60)),(y+40),10,10),None,None]
            self.high_kick_boxes = [None,None,None,pygame.Rect((x+ (self.hitbox_width - 35)),(y+25),10,10),None,None,None]

        if self.reversed:
            x -= self.hitbox_width
            
            self.punch_boxes = [None,None, None, pygame.Rect((x + (self.hitbox_width)),(y + 55),10,10), None]
            self.kick_boxes = [None,pygame.Rect((x+100),(y+(self.hitbox_height - 45)),10,10),pygame.Rect((x+ 110),(y+30),10,10),None, None]
            self.chop_boxes = [None,None,None,pygame.Rect((x+(self.hitbox_width - 5)),(y+60),10,10),pygame.Rect((x+(self.hitbox_width + 3)),(y+60),10,10),None]
            self.low_punch_boxes = [None,None,pygame.Rect((x+90),(y+50),10,10),pygame.Rect((x+100),(y+50),10,10),None]
            self.low_kick_boxes = [None,None,pygame.Rect((x+(self.hitbox_width+5)),(y+ (self.hitbox_height + 15)),10,10),None,None,None]
            self.uppercut_boxes = [None,None,pygame.Rect((x+self.hitbox_width),(y+65),10,10),pygame.Rect((x+(self.hitbox_width + 30)),(y+40),10,10),None,None]
            self.high_kick_boxes = [None,None,None,pygame.Rect((x+ (self.hitbox_width - 15)),(y+25),10,10),None,None,None]
...
class Frieza_enemy(Enemy, Frieza):
    ...
    def set_queue(self, moves, player):
        ...
        if self.check_player_position(player):
            ...
            elif 0.5 <= chance < 0.7 and self.close_contact_check(player):
                block.set_priority(0)
                default.set_priority(1) 
                
            elif chance >= 0.7 and not self.close_contact_check(player):
            ...
            if chance > 0.2 and not self.check_player_position(player):
...
class Cell_enemy(Enemy, Cell):
    def __init__(self, target):
        Enemy.__init__(self, "Cell", target)
        Cell.__init__(self,target)
        self.level = 3
        self.value = 180
        self.icon = icon(self)
    ...
    def set_smaller_detectbox(self,x,y):
        ...
        self.smaller_detectbox = pygame.Rect(x,y,(self.s_detectbox_width + 40), self.s_detectbox_height)
    ...
    def set_queue(self,moves,player):
        ...
        #set the priority of the action objects

        chance = random.uniform(0,1)

        if self.get_health() > 50:
            #move towards the player if not already close
            if not self.close_contact_check(player):
                if player.get_reversed():
                    right.set_priority(0)
                else:
                    left.set_priority(0)

            #if they are close, choose between different close combos
            else:
                #different chance brackets so that some moves can be predicted
                if 0 < chance < 0.4:
                    kick.set_priority(0)
                    punch.set_priority(1)
                    jump.set_priority(2)
                    default.set_priority(3)
                    ...
                    punch.set_priority(0)
                    kick.set_priority(2)
                    crouch.set_priority(1)
                    block.set_priority(3)
            ...
            if self.check_player_position(player) and self.close_contact_check(player):
                if 0 < chance < 0.6:
                    if player.get_reversed():
                        left.set_priority(0)
                    else:
            ...
            elif not self.close_contact_check(player):
                #larger chance of block; small chance of kamehameha

                #increases the chance for the top chance bracket if cell if in the corner
                if self._getx() < 100 or self._getx() > 900:
                    chance += 0.4
                    if chance > 1:
                        chance = 1
            ...
            elif self.check_player_position(player):
                default.set_priority(0)
